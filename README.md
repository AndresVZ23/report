...
Para esta sección seleccionamos aquellos cuya ejecución involucra varios componentes, atraviesa distintos bounded contexts o introduce restricciones de rendimiento, seguridad o disponibilidad relevantes. Un criterio práctico que nos ayudó fue preguntarnos: "si este user story se modificara profundamente, ¿cambiaría la forma del sistema?". Cuando la respuesta era afirmativa, lo trasladábamos al backlog arquitectónico.

| Epic / User Story ID | Título | Descripción | Criterios de Aceptación | Relacionado con (Epic ID) |
|---|---|---|---|---|
| EP01 | Gestión del Gemelo Digital | Contenedor de historias orientadas a la construcción, sincronización y visualización del gemelo digital 3D del estacionamiento. | — | — |
| US01 | Visualización 3D del estado de ocupación | Como operador del estacionamiento deseo visualizar el modelo 3D del gemelo digital con el estado de ocupación por plaza para monitorear el uso del espacio en tiempo casi real. | **Dado** que el operador inició sesión en el panel web, **cuando** accede al módulo de operaciones, **entonces** se muestra el modelo 3D con las plazas coloreadas según su estado (libre, ocupada, reservada, fuera de servicio). **Dado** un cambio de ocupación detectado por un sensor, **cuando** transcurren menos de dos segundos, **entonces** el color de la plaza en el modelo 3D refleja el nuevo estado. | EP01 |
| US02 | Alerta espacial por detección de humo | Como operador deseo recibir una alerta georreferenciada sobre el modelo 3D cuando un sensor detecte humo para coordinar una respuesta inmediata. | **Dado** un evento de humo generado por el simulador, **cuando** el sistema lo recibe, **entonces** se destaca visualmente la zona afectada en el modelo 3D y se genera una notificación sonora y visual en el panel. **Dado** la alerta activa, **cuando** el operador la reconoce, **entonces** se registra el acuse con marca temporal y usuario. | EP01 |
| EP02 | Experiencia del Conductor | Contenedor de historias orientadas a la interacción del conductor con la solución a través de la aplicación móvil. | — | — |
| US03 | Consulta de disponibilidad por zona | Como conductor deseo consultar la disponibilidad de plazas libres por zona o nivel para decidir dónde estacionar al ingresar al centro comercial. | **Dado** que el conductor abre la aplicación móvil, **cuando** selecciona un centro comercial, **entonces** se presenta la cantidad de plazas libres por zona con una antigüedad menor a cinco segundos. **Dado** que no existe conectividad, **cuando** el conductor abre la aplicación, **entonces** se muestra la última información cacheada con el sello de tiempo correspondiente. | EP02 |
| US04 | Registro y localización del vehículo | Como conductor deseo registrar la ubicación donde estacioné mi vehículo para encontrarlo con facilidad al regresar. | **Dado** que el conductor finalizó su maniobra de estacionamiento, **cuando** confirma la ubicación en la aplicación, **entonces** el sistema guarda la referencia (zona, nivel, plaza) asociada al usuario. **Dado** el registro previo, **cuando** el conductor solicita ubicar su vehículo, **entonces** se muestra un indicador visual sobre un plano del nivel correspondiente. | EP02 |
| US05 | Notificación de incidentes de seguridad | Como conductor deseo recibir una notificación cuando se declare un incidente de seguridad en la zona donde se encuentra mi vehículo para tomar precauciones o seguir instrucciones de evacuación. | **Dado** un incidente confirmado en una zona, **cuando** el sistema identifica vehículos registrados en esa zona, **entonces** envía una notificación con indicaciones breves de evacuación a cada conductor afectado en menos de cinco segundos. | EP02 |
| EP03 | Plataforma de Simulación y Telemetría | Contenedor de historias técnicas relacionadas con la ingesta, simulación y distribución de eventos de sensores. | — | — |
| TS01 | Ingesta de telemetría simulada | Como developer deseo exponer un endpoint y un canal de mensajería para que el simulador de sensores envíe eventos de ocupación, humo, flujo vehicular y luminosidad al sistema. | **Dado** un evento válido, **cuando** se envía al canal de ingesta, **entonces** el servicio responde con 202 Accepted en menos de 500 ms. **Dado** un evento con formato inválido, **cuando** se envía al canal, **entonces** el servicio responde con 400 Bad Request y el evento no se propaga al gemelo digital. | EP03 |
| TS02 | Sincronización con Azure Digital Twins | Como developer deseo propagar los eventos válidos hacia la instancia de Azure Digital Twins para mantener actualizado el estado del gemelo. | **Dado** un evento válido ya persistido en el servicio de telemetría, **cuando** se dispara el proceso de sincronización, **entonces** se actualiza la propiedad correspondiente del twin en menos de 1.5 segundos. | EP03 |
| TS03 | Control de acceso basado en roles | Como developer deseo implementar autenticación y autorización diferenciada para operadores y conductores, de modo que cada segmento acceda solo a las funcionalidades pertinentes. | **Dado** un usuario con rol de conductor, **cuando** intenta acceder al panel de operaciones, **entonces** el sistema responde con 403 Forbidden. **Dado** un usuario con rol de operador, **cuando** solicita un endpoint de operaciones, **entonces** el token JWT es validado y se concede el acceso. | EP03 |
...
|---|---|---|---|---|---|---|
| Performance | Simulador de sensores IoT | Llega un evento de cambio de ocupación | Servicio de ingesta + Azure Digital Twins | Operación normal con carga proyectada (hasta 1,200 eventos/minuto) | El sistema valida, persiste y propaga el evento al gemelo digital | El estado del twin se actualiza en menos de 2 segundos en el percentil 95 |
| Availability | Operador del estacionamiento | Intenta acceder al panel web de operaciones | Panel web + API Gateway | Operación en horario comercial (06:00–23:00) | El sistema responde con la vista del gemelo digital disponible | Disponibilidad mensual ≥ 99.5% durante horario de operación |
| Usability | Conductor en el ingreso al centro comercial | Solicita conocer la disponibilidad por zona | Aplicación móvil (App Inventor) | Conductor en movimiento, red móvil variable | La aplicación muestra la disponibilidad por zona | El conductor obtiene la información útil en menos de 5 segundos desde abrir la app, en el percentil 90 |
| Security | Actor externo no autorizado | Envía una petición a endpoints de operaciones sin token válido | API Gateway + servicios de backend | Operación normal | El sistema rechaza la petición y registra el intento | 100% de peticiones sin token válido son respondidas con 401/403 y 100% de intentos quedan registrados |
| Modifiability | Equipo de desarrollo | Se requiere incorporar un nuevo tipo de sensor (por ejemplo, CO₂) | Servicio de telemetría + modelo DTDL | Ciclo de release regular | Se agregan los artefactos necesarios para soportar el nuevo tipo | El cambio se completa en menos de 5 días-persona y no requiere modificar otros bounded contexts |
| Interoperability | Aplicación móvil de terceros (futura) | Solicita información de disponibilidad vía API pública | API pública de disponibilidad | Internet abierto | El sistema expone los datos según contrato OpenAPI publicado | El 100% de las peticiones válidas recibe respuesta conforme al contrato versión 1.0 |
| Scalability | Operador de la plataforma | Se incorpora un nuevo estacionamiento con 500 plazas adicionales | Modelo del gemelo digital + base de datos | Crecimiento gradual de clientes | El sistema acepta el nuevo estacionamiento sin degradación perceptible | El tiempo de respuesta promedio del panel se mantiene por debajo de los 2 segundos tras la incorporación |
